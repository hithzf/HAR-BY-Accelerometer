package com.example.hzf.recognition.feature;
...
import com.example.hzf.recognition.classifier.Center;
...
//        LogUtil.d(FeatureCore.class.getSimpleName(), "fea = " + combineList.toString());
        ...
        //预分类
        int centerResult = 0;
        if(type == MainActivity.ALGORITHM_ADAPTIVE){
            //预分类
...
//        }
        ...
        int saveId = 0;
        if(saveDB) {
            LogUtil.d(FeatureCore.class.getSimpleName(), "startTime = " + startTime + ",endTime = " + endTime);
        ...
        //原始算法
        if(type == MainActivity.ALGORITHM_ORIGIN) {
            final int finalSaveId1 = saveId;
            SVM.exec.execute(new Runnable() {
                @Override
                public void run() {
                    SVM.classify(combineList, finalSaveId1, OriginService.SAMPLE_FREQ);
                }
            });
        }
        else if(type == MainActivity.ALGORITHM_ADAPTIVE){
            if(centerResult == 0){
                final int finalSaveId = saveId;
                SVM.exec.execute(new Runnable() {
                    @Override
                    public void run() {
                        SVM.classify(combineList, finalSaveId, AdaptiveService.BEST_FREQ);
                    }
                });
            }
        }
