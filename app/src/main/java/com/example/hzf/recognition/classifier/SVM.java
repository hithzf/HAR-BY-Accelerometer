package com.example.hzf.recognition.classifier;
...
public class SVM {
    ...
    public static void calculate(ArrayList<Integer> copyList) {
        LogUtil.d(SVM.class.getSimpleName(), copyList.toString());
        //将待分类数据保存到文件
        FileOperateUtil.writeFeaVector(copyList);
        //分类
        forecast(OriginService.SAMPLE_FREQ);
        //读取文件并记录分类结果
    ...
    public static void forecast(int freq){
        String modelName = "";
        if(freq == AdaptiveService.BEST_FREQ){
            modelName = "model20.txt";
        }else if(freq == OriginService.SAMPLE_FREQ){
