...
public class FileOperateUtil {
    ...
    public static List<String> readCluster(){
        ...
        try {
            ...
            while ((readLine = bufferedReader.readLine()) != null){
                if(!readLine.isEmpty()){
                    LogUtil.d(FileOperateUtil.class.getSimpleName(), readLine);
                    res.add(readLine);
                }
            }
            bufferedReader.close();
    ...
    public static void saveMemoryAndTime(int lastIndex, int mode){
        ...
        String time = df.format((double) fSum / MainActivity.GROUP_NUM + (double) cSum / MainActivity.GROUP_NUM);

        LogUtil.d(FileOperateUtil.class.getSimpleName(), "mode = " + mode);
        MemoryTime memoryTime = new MemoryTime(lastIndex, time, svmFreq, timeList.size(), mode);
        if(memoryTime.save()){
            LogUtil.d(FileOperateUtil.class.getSimpleName(), "memoryTime saved");
            timeList.clear();
        }
    }
